...
<%--修改新闻--%>
<section class="row" style="width: 100%;margin: 0 auto;background: url(img/newbacktwo.jpg)">
    ...
    <%
        LookMovieTypeDao lookNewsDao=new LookMovieTypeDaoImpl();
        List<MovieType> list=lookNewsDao.look();
    %>
...
<%--                <input type="text" class="form-control" id="id" placeholder="类型编号" name="id">--%>
                <select type="text" class="form-control" id="id" placeholder="类型编号" name="id">
                    <%
                        for (MovieType MovieType : list) {
                    %>
                    <option><%=MovieType.getId()%></option>
                    <%
                        }
                    %>
                </select>
