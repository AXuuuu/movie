...
<body style="background-image: url(img/newbacktwo.jpg)">
<!-- ##### Header Area Start ##### -->
...
<section class="row" style="width: 100%;margin: 0 auto;background: url(img/main-bg.png)">
    ...
    <%
        LookNewsTypeDao lookNewsDao=new LookNewsTypeDaoImpl();
        List<NewsType> list=lookNewsDao.look();
    %>
    ...
    <div class="col-md-8 col-md-offset-2">
        <form action="UpdateNewsTypeAction.action" class="fh5co-form animate-box" data-animate-effect="fadeInRight" method="post" style="border-radius: 20px">
            <div class="form-group">
                <label for="id" class="sr-only">类型编号</label>
<%--                <input type="text" class="form-control" id="id" placeholder="类型编号" name="id">--%>
                <select type="text" class="form-control" id="id" placeholder="新闻类别" name="id">
                    <%
                        for (NewsType newsType : list) {
                    %>
                    <option><%=newsType.getId()%></option>
                    <%
                        }
                    %>
                </select>
